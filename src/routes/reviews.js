...
router.post('/portal/patient/case/:caseId/review', requireRole('patient'), async function(req, res) {
  try {
    ...
    var isAnonymous = req.body.is_anonymous === '1' || req.body.is_anonymous === 1 ? true : false;
    ...
    await execute(
      ...
      [reviewId, caseId, patientId, order.doctor_id, rating, reviewText || null, isAnonymous, now, now]
    );
...
router.put('/portal/patient/review/:reviewId', requireRole('patient'), async function(req, res) {
  try {
    ...
    var review = await safeGet('SELECT * FROM reviews WHERE id = $1', [reviewId], null);
    ...
    await execute(
      'UPDATE reviews SET rating = $1, review_text = $2, is_anonymous = $3, updated_at = $4 WHERE id = $5',
      [rating, reviewText || null, isAnonymous, new Date().toISOString(), reviewId]
    );
...
router.get('/portal/doctor/:doctorId/reviews', async function(req, res) {
  try {
    var doctorId = String(req.params.doctorId).trim();
    var lang = res.locals.lang || 'en';
    var isAr = lang === 'ar';

    var doctor = await safeGet('SELECT id, name, specialty_id FROM users WHERE id = $1 AND role = $2', [doctorId, 'doctor'], null);
    if (!doctor) return res.status(404).send(isAr ? 'الطبيب غير موجود' : 'Doctor not found');

    var specialty = doctor.specialty_id ? await safeGet('SELECT name FROM specialties WHERE id = $1', [doctor.specialty_id], null) : null;

    var reviews = await safeAll(
    ...
    // Rating distribution
    var distribution = await safeAll(
      'SELECT rating, COUNT(*) as count FROM reviews WHERE doctor_id = $1 AND is_visible = true GROUP BY rating ORDER BY rating DESC',
      [doctorId], []
    );
...
router.get('/api/doctors/:doctorId/rating', async function(req, res) {
  try {
    var doctorId = String(req.params.doctorId).trim();
...
router.delete('/portal/admin/review/:reviewId', requireRole('admin', 'superadmin'), async function(req, res) {
  try {
    var reviewId = String(req.params.reviewId).trim();
    var action = req.body.action || 'hide'; // 'hide' or 'flag'

    if (action === 'flag') {
      await execute('UPDATE reviews SET admin_flagged = true, updated_at = $1 WHERE id = $2', [new Date().toISOString(), reviewId]);
      return res.json({ ok: true, message: 'Review flagged' });
    } else {
      await execute('UPDATE reviews SET is_visible = false, updated_at = $1 WHERE id = $2', [new Date().toISOString(), reviewId]);
...
router.get('/admin/reviews', requireRole('admin', 'superadmin'), async function(req, res) {
  try {
    var lang = res.locals.lang || 'en';
    var isAr = lang === 'ar';
...
router.get('/portal/patient/reviews', requireRole('patient'), async function(req, res) {
  try {
    var patientId = req.user.id;
    var lang = res.locals.lang || 'en';
    var isAr = lang === 'ar';
    ...
    var pendingCases = await safeAll(
      `SELECT o.id, o.status, o.created_at, d.name as doctor_name, s.name as specialty_name
       ...
       WHERE o.patient_id = $1 AND LOWER(o.status) IN (${placeholders})
         AND o.id NOT IN (SELECT order_id FROM reviews WHERE patient_id = $${completedStatuses.length + 2})
