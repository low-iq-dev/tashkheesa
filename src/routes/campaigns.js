...
router.get('/portal/admin/campaigns', requireRole('admin', 'superadmin'), async function(req, res) {
  try {
    var lang = res.locals.lang || 'en';
    var isAr = lang === 'ar';

    var campaigns = await queryAll(
...
router.get('/portal/admin/campaigns/new', requireRole('admin', 'superadmin'), async function(req, res) {
  try {
    var lang = res.locals.lang || 'en';
    var isAr = lang === 'ar';
...
router.post('/portal/admin/campaigns', requireRole('admin', 'superadmin'), async function(req, res) {
  try {
    var lang = res.locals.lang || 'en';
    var isAr = lang === 'ar';
    ...
    var recipientCount = await populateRecipients(id, targetAudience);
...
router.get('/portal/admin/campaigns/:id', requireRole('admin', 'superadmin'), async function(req, res) {
  try {
...
router.post('/portal/admin/campaigns/:id/send', requireRole('admin', 'superadmin'), async function(req, res) {
  try {
    var campaignId = String(req.params.id).trim();
    var campaign = await queryOne('SELECT * FROM email_campaigns WHERE id = $1', [campaignId]);
...
router.post('/portal/admin/campaigns/:id/cancel', requireRole('admin', 'superadmin'), async function(req, res) {
  try {
    var campaignId = String(req.params.id).trim();
...
router.get('/unsubscribe/:token', async function(req, res) {
  try {
    ...
    await execute('UPDATE users SET email_marketing_opt_out = true WHERE id = $1', [userId]);
...
async function populateRecipients(campaignId, audience) {
  ...
  switch (audience) {
    case 'patients':
      query = "SELECT id, email FROM users WHERE role = 'patient' AND is_active = true AND email_marketing_opt_out = false AND email IS NOT NULL";
      break;
    case 'doctors':
  ...
  var users = await queryAll(query, []);
  var now = new Date().toISOString();
  ...
  await withTransaction(async function(client) {
    for (var i = 0; i < users.length; i++) {
      var u = users[i];
      await client.query(
        'INSERT INTO campaign_recipients (id, campaign_id, user_id, email, status, created_at) VALUES ($1, $2, $3, $4, $5, $6)',
        [randomUUID(), campaignId, u.id, u.email, 'pending', now]
      );
    }
  });
...
async function processCampaign(campaignId) {
  ...
  var recipients = await queryAll(
    "SELECT * FROM campaign_recipients WHERE campaign_id = $1 AND status = 'pending' LIMIT 1000",
    [campaignId]
  );
  ...
  function sendNext() {
    if (idx >= recipients.length) {
      ...
      execute("UPDATE email_campaigns SET status = 'sent', sent_at = $1, total_sent = $2, total_failed = $3 WHERE id = $4",
        [new Date().toISOString(), sentCount, failedCount, campaignId]).catch(function() {});
      return;
    }
    ...
    // Determine user lang
    queryOne('SELECT lang FROM users WHERE id = $1', [r.user_id]).then(function(userRow) {
      userRow = userRow || { lang: 'en' };
      ...
      sendEmailFn({
        ...
        if (result && result.ok) {
          execute("UPDATE campaign_recipients SET status = 'sent', sent_at = $1 WHERE id = $2", [new Date().toISOString(), r.id]).catch(function() {});
          sentCount++;
        } else {
          execute("UPDATE campaign_recipients SET status = 'failed', error = $1 WHERE id = $2", [(result && result.error) || 'Send failed', r.id]).catch(function() {});
          failedCount++;
        }
        // Rate limit: ~5/sec
        setTimeout(sendNext, 200);
      }).catch(function(err) {
        execute("UPDATE campaign_recipients SET status = 'failed', error = $1 WHERE id = $2", [err.message || 'Error', r.id]).catch(function() {});
        failedCount++;
        setTimeout(sendNext, 200);
      });
    }).catch(function() {
