...
- Connected copy fixes (same day, second commit): the intake response drops its committed hour ("Our team will contact you shortly." — no figure), and notifyCaseReceived is passed slaCfg.sla_hours so the email states the real window and its 4/18/48 band names the tier (it fell back to a hardcoded "within 48 hours"). Arabic side checked: this inline email HAS no Arabic variant (EN-only helper; intake hardcodes language 'en'); the bilingual case-submitted.hbs prints dynamic {{slaHours}} — clean. Recorded for Ziad: Arabic-speaking web leads get an English email. Guards: checks (3)-(5) in the same test file, red-first. Suite: Passed 1914 / Failed 6 / Skipped 52.
